...
- **Temporal core** — `core/{time,temporal_context,node_cache,annotations,scheduler,strips,strip_access,strip_resolve,subscriptions,environment,cycle_validator,events}.py`. Tests under `backend/tests/temporal/` (pydantic, pytest, and pytest-asyncio only; no DB: `pip install pydantic pytest pytest-asyncio`).
- **Substrate core (nodecules v2)** — `core/descriptions.py` + `core/assay_metrics.py` (PR-d1), `core/placement.py` (PR-d2), `core/pmap.py` + `core/store.py` (PR-r3a: the node store — manifests, CAS transactions with a per-scope resolution policy, envelopes, residency, composed hash), `core/timeline.py` (timebases, timelines, skew maps), `core/generation.py` (PR-r5a: produce node X over the store — four outcomes, envelopes as receipts, plans dispatched), `core/replica.py` (the CRDT basis: manifests as a DAG, deterministic merge, sync), `core/scene.py` (presentation time, expiry, transition hints, the frame as a pure function, the simultaneity bound), `core/strip_nodes.py` (a strip as one node, the small form), `core/llm_realization.py` (a provider as a realization), `core/table.py` (the table: participants, attention with expiry, optional following, field-wise open collaboration), `core/disk.py` (the disk tier: sparse load, write-through, integrity), `core/activation.py` (markers and activation as a forward commit), `core/tracking.py` (the event log outside the store; lineage), `core/decisions.py` (decisions and their presentations; a business node advances once), `core/deferral.py` (a production without a realization here becomes a request node another agent fulfils). Same test tree, same no-DB rule.
- **Demos** — `demos/` holds runnable consumers of the substrate (founder direction 2026-09-13: a chat with a live configurable graph, a table chat, a stenota remake, a toy keyhole; 2026-09-15: a table with open collaboration and attention runs; 2026-09-17: the inspector, a local web app to look at, edit, produce, activate, and watch a store; 2026-09-18: the table chat, a decision presented three ways and a deferred model step). Nothing in `core/` imports them; each says what it does not show. Run from `backend/` with `PYTHONPATH=.`.
