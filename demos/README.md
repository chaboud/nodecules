...
| `table_chat.py` | **a chat with a table**: one decision (sixteen destinations) presented three ways — a phone four at a time as a tournament, a wall all at once, a screen reader in spoken form — answered through an input strip, the business graph advancing exactly once; then a step that needs a model nobody here has becomes a **request node** that a second agent fulfils, and back here it is a cache hit | runs; `--store DIR` to open the result in the inspector |
