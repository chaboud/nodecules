...
# Nodecules Claude Integration Test Script
# This script tests the complete Claude API integration workflow

set -e  # Exit on any error

BASE_URL="http://localhost:8000/api/v1"
TEST_MESSAGE="Hello Claude! Please respond with exactly: 'Integration test successful!'"

echo "🧪 Nodecules Claude Integration Test"
echo "===================================="
...
echo ""
echo "2. ✅ Checking Claude API configuration..."
if ! docker-compose exec -T backend env | grep -q "ANTHROPIC_API_KEY"; then
...
# Check available nodes and verify anthropic provider
echo ""
echo "3. ✅ Checking available LLM providers..."
if ! curl -s "$BASE_URL/plugins/nodes/immutable_chat" | grep -q "anthropic"; then
    echo "❌ Anthropic provider not available in immutable_chat node"
    exit 1
fi
echo "   Anthropic provider available ✅"

# Create test graph
echo ""
echo "4. ✅ Creating Claude test graph..."
GRAPH_RESPONSE=$(curl -s -X POST "$BASE_URL/graphs/" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Claude Integration Test",
    "description": "Automated test of Claude API integration",
    "nodes": {
      "input_1": {
        "node_id": "input_1",
        "node_type": "input",
...
GRAPH_ID=$(echo "$GRAPH_RESPONSE" | python3 -c "import sys, json; print(json.load(sys.stdin)['id'])" 2>/dev/null || echo "Claude Integration Test")
echo "   Test graph created: $GRAPH_ID ✅"

# Execute graph with Claude
echo ""
echo "5. ✅ Testing Claude API execution..."
...
CLAUDE_RESPONSE=$(echo "$EXECUTION_RESPONSE" | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    outputs = data.get('outputs', {})
    print(outputs.get('claude_response', 'No response found'))
except:
...
echo "   • Graph creation"
echo "   • Claude API execution"
echo "   • Context data output"
echo "   • Context supremacy model"
echo ""
echo "🚀 Your Claude integration is working correctly!"
echo "   You can now create graphs using provider: 'anthropic'"
